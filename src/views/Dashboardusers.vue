<template>
  <body style="background-color: #cccccc">
    <div>
      ...
      <div style="font-weight: 800">userId</div>
      <div class="userId" v-for="order in orders" :key="order._id">
        {{ order.userId }}
      </div>
      <div style="margin: 15px">
        <div style="font-weight: 800">inviteId</div>
        <div
          @click="gotouser"
      ...
      <div class="percentage">
        <div style="font-weight: 800">usdtId</div>
        <h1
          style="display: inline-block"
          v-for="user in orders"
          :key="user._id"
...
export default {
  ...
  async created() {
    ...
    this.inviteuser = response.data.userinviteId;
    console.log(this.orders);
  },
  methods: {
    gotouser() {
      this.$router.push(`/admin/Dashboardusers/${this.inviteuser}`);
    },
    async profitcrdet() {
      this.sof = 1;
      if (this.profitcrde <= 0) {
        this.alert = "zero or less not allawed";
      } else {
        const orderId = this.id;
    ...
    async percentageUsersa() {
      const orderId = this.id;
      await AuthenticationServices.percentageUsersbalance({
        userId: orderId,
        balance: this.percentageUsers,
      });
      window.location.reload();
    },
    async usdtIdUserschanhe() {
      const orderId = this.id;
      await AuthenticationServices.usdtIdurluser({
        userId: orderId,
        balance: this.usdtId,
      });
      window.location.reload();
